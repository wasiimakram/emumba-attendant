...
import { attemptLogin, selectAuthError, selectAuthSuccess, selectAuthUser, selectIsFirstLogin, selectIsLoggedIn } from '../../app-redux/modules/auth/authSlice';
...
export default function Login() {
    ...
    const handleSubmitForm = async (values: IUser) => {
        const updatedValues = {...values,type: pathname === '/admin-login' ? 'admin' : 'user'}
    ...
    useEffect(() => {
        if (loginAttempted) {
            if (!isLoggedIn) {
                ...
                if(authUser.type === 'user'){
                    isFirstLogin ? history.push("/change-password")
      ...
      }, [loginAttempted,isLoggedIn,authError,authSuccess,history,isFirstLogin,authUser]);
  return (
    <>
                ...
                <Content className='form-wrap'>
                    ...
                    <Form.Item>
                        ...
                        <Content className='icons-wrap'>
                            Or <Text>Sign in using social platforms</Text><br />
                        ...
                        <Content className='admin-link'>
                            {!isUser ? (<>
                                Or Are you a User? <Link to="/login">SignIn as User instead?</Link>
                                        </>) : 
                                (<>
                                    Or Are you an Admin? <Link to="/admin-login">SignIn as Admin instead?</Link>
                                </>)
                            }
                        </Content>
                    </Form.Item> 
